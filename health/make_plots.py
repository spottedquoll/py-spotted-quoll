...
import numpy as np
from quoll.utils import flatten_list, key_match_in_dicts
...
if plot_4:
    ...
    plt.savefig(work_dir + 'figs/fig4b_bar_chart.png', dpi=700, bbox_inches='tight')
    ...
    share_values = df1[last_category].values
    for idx, x in enumerate(x_labels):
        ax.text(idx-0.2, yearly_totals[idx] + .25, str(round(share_values[idx], 2)) + '%')
